...
(() => {
    ...
    function showStateInfo(stateAbbr) {
        ...
        const rxLevel = stateData.rx_percentage >= 68 ? "Very High (Crisis)" :
                       stateData.rx_percentage >= 66 ? "High" :
                       stateData.rx_percentage >= 60 ? "Moderate" : "Lower";
    ...
    function renderPlants() {
        const filtered = plantsData.plants.filter(p => 
            matchesZone(p) && matchesConditions(p) && matchesQuery(p)
        );
        ...
        resultsGrid.innerHTML = filtered.map(plant => `
